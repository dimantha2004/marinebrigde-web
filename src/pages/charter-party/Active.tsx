...
export default function CharterActive() {
  ...
  const activeOrders = useMemo(
    () =>
      orders.filter((o: Order) =>
        ['active', 'in_execution', 'pending_port_approval'].includes(o.overall_status)
      ),
    [orders]
  );
    ...
    <Box>
      ...
      {isLoading ? (
        ...
        <Box sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography sx={{ color: palette.hullGray, mt: 1 }}>Couldn't load active orders.</Typography>
        </Box>
      ) : activeOrders.length === 0 ? (
        <Box sx={{ textAlign: 'center', py: 6 }}>
          <DirectionsBoatOutlined sx={{ color: palette.hullGray, fontSize: 36 }} />
