...
export default function CaptainOrderDetail() {
  ...
  return (
    <Box>
          ...
          return (
            <Accordion key={li.id} disableGutters sx={{ mb: 1, bgcolor: palette.oceanMid, '&:before': { display: 'none' } }}>
              ...
              <AccordionDetails sx={{ bgcolor: palette.navyDeep }}>
                ...
                {li.unit_price != null && <DetailLine label="Unit Price" value={`$${li.unit_price.toFixed(2)}`} />}
                {li.total_price != null && <DetailLine label="Line Total" value={`$${li.total_price.toFixed(2)}`} />}
                {li.supplier_decline_reason && (
                  <DetailLine label="Decline Reason" value={li.supplier_decline_reason} />
                )}
                <Button
                  variant="outlined"
      ...
      {documents.length === 0 ? (
        <Typography sx={{ color: palette.hullGray, mb: 1 }}>No documents yet.</Typography>
      ) : (
        documents.map((doc) => (
      ...
      <Box sx={{ mt: 3, display: 'flex', flexDirection: 'column', gap: 1, maxWidth: 360 }}>
        ...
        {canCancel && (
          ...
          >
            {cancelling ? <CircularProgress size={20} sx={{ color: palette.alertRed }} /> : 'Cancel Order'}
          </Button>
        )}
      </Box>
